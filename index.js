...
var argv = require("yargs")
  ...
  .describe("u", "an organization override for GitHub URLs").argv;
...
getParams(function (err, params) {
  ...
  readdirp({
    ...
    if (!argv.t) {
      if (file.path === "index.js" || file.path === "_test.js") return;
    }
    ...
    fs.readFile(file.fullPath, "utf8", function (err, content) {
      ...
      if (file.name.match(/\_\.editorconfig$/g))
        dest = dest.replace("_.editorconfig", ".editorconfig");
      if (file.name.match(/\_\.travis.yml$/g))
        dest = dest.replace("_.travis.yml", ".travis.yml");
...
function getParams(done) {
  conf.load({}, function (err, config) {
    ...
    if (typeof argv.u === "string") {
      ...
      console.log(
        chalk.green(
          "Creating module under organization " + chalk.bold(data.org.name)
        )
      );
    } else if (argv.u) {
    ...
    prompt(
      ...
      function (results) {
        ...
        results.testDescription = escape(results.description).replace(
          /\\"+/g,
          '"'
        );
        results.description = dequote(results.description);
        results.varName = varName(results.name);
        results.tags = JSON.stringify(
          results.tags
            .split(" ")
            .map(function (str) {
        ...
        if (argv.t) {
          handleInstall(function (err, dep) {
            if (err)
              console.log(
                chalk.red("Error installing " + TEST_RUNNER + " " + err)
              );
            else {
              //prefix for --save-dev
...
function bail(cmd) {
  console.log("");
  console.log(
    "Missing configuration option, please run the following using your own value:"
  );
  console.log("");
  console.log("  > " + cmd);
  console.log("");
